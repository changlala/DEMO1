package com.example.demo1.camera;
...
import com.example.demo1.TakePhotoActivity;
...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback ,Camera.AutoFocusCallback{
    ...
    private void setParameters(Camera.Parameters p) {
        ...
        p.setPreviewSize(previewSize.width, previewSize.height);
        p.setPictureSize(previewSize.width, previewSize.height);
        Log.d("demo1", "setParameters: 拍摄照片大小"+previewSize.width+" "+previewSize.height);
        p.setFocusMode(Camera.Parameters.FOCUS_MODE_AUTO);

        if (TakePhotoActivity.isTransverse) {
